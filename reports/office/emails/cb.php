<?php

	// Office List vvv
	// oper_mgr.php
	$operMgr .= "";

	// area_exp_mgr.php
	$areaExpMgr .= "";

	// region_exp_mgr.php
	$regExpMgr .= "";

	// region_qual_mgr.php
	$regQualMgr .= "";

	// exp_mgr.php
	$expMgr .= "";

	// cust_rltns_mgr.php
	$custRltnsMgr .= "";

	// rltn_cent_sup.php
	$rltnCentSup .= "caitlin@waterworksswim.com";

	// shift_lead_sr.php
	$shiftLeadSr .= ", caitlin@waterworksswim.com";

	// shift_lead.php
	$shiftLead .= ", caitlin@waterworksswim.com";

	// train_lead_sr.php
	$trainLeadSr .= ", caitlin@waterworksswim.com";

	// train_lead.php
	$trainLead .= ", caitlin@waterworksswim.com, rebecca.h@waterworksswim.com";

	// cee_sr.php
	$ceeSr .= "caitlin@waterworksswim.com, rebecca.h@waterworksswim.com";

	// cee.php
	$cee .= "caitlin@waterworksswim.com, rebecca.h@waterworksswim.com";

	// rltn_spec.php
	$rltnSpec .= "caitlin@waterworksswim.com, david.p@waterworksswim.com, lisa@waterworksswim.com";

	// proj_spec.php
	$projSpec .= "caitlin@waterworksswim.com, david.p@waterworksswim.com, lisa@waterworksswim.com";

	// live_chat.php
	$chat .= "";

	// call_list.php
	$callList .= "";

	// host.php
	$host .= " caitlin@waterworksswim.com";
	// Office List ^^^

	// trainee.php
	$trainee .= ", caitlin@waterworksswim.com, rebecca.h@waterworksswim.com";

	// scomp.php
	$scomp .= ", caitlin@waterworksswim.com, justin@waterworksswim.com";

	// event_host.php
	$eventHost .= "";

	// event_coor.php
	$eventCoor .= "";

	// event_comm.php
	$eventComm .= "";

	// acc_mgr.php
	$accMgr .= "";

	// acc.php
	$acc .= "";

	// emp_exp_admin.php
	$eea .= "";

	// emp_exp_assist.php
	$eeaa .= "";

?>
